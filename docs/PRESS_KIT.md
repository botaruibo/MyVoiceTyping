...
相比成熟商业语音输入产品，MyVoiceTyping 仍处于早期；它的差异在于开源、本地优先、0 费用，并且应用、文本润写模型和 ASR 后处理数据集都公开。后续方向是 self-evaluation：用户确认后的改写结果可以作为本地偏好样本，用于评估或调优本地模型，让它更贴合个人表达习惯。
```
...
- App repo license 仍在确认中，进展见 <https://github.com/botaruibo/MyVoiceTyping/issues/7>；
