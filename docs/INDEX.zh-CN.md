...
| Self-evaluation / 自进化 | [Self-evaluation](./SELF_EVALUATION.md) | 说明用户确认后的改写结果如何作为本地偏好样本，用于评估或调优本地模型 |

注意：不要把 self-evaluation 理解成默认上传用户隐私数据。它的目标是让用户确认后的文本在本地可控边界内成为偏好样本，让本地模型逐步贴近个人表达习惯。
...
- App repo license 仍在确认中：<https://github.com/botaruibo/MyVoiceTyping/issues/7>
