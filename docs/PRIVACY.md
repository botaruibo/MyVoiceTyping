...
MyVoiceTyping 从 alpha-0.03 开始重点推进 self-evaluation / 自进化：用户在语音输入后对结果进行修改、确认，这些“原始转写 → 用户确认文本”的样例可以写入本机 `voice_history.jsonl`，帮助本地 LLM 越用越贴合个人表达习惯。

这类数据比普通日志更敏感，因为它可能包含用户真实工作内容、常用术语、写作风格和个人表达偏好。因此设计上应遵守以下边界：

- 默认保留在用户本机，不自动上传到云端；
- 应由用户明确控制是否保存、删除、导出或参与本地训练；
- 用于本机 MLX LoRA 训练或轻量调优前，应支持用户预览和清理样例；
- 调优完成后转换为 GGUF Q4 替换本地模型时，应先做完整性检查；
- 不会自动进入公开数据集；
...
更具体的样例格式、用户控制边界和当前完成度见：[Self-evaluation / 本地自进化说明](./SELF_EVALUATION.md)。
