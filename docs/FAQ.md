...
Self-evaluation / 自进化是 MyVoiceTyping 的一个长期方向：语音输入生成初稿后，用户最终如何改写、如何确认，可以沉淀为本地偏好数据。后续这些样本可以先用于本地评估、热词和规则优化；在用户明确授权后，再探索本地 LLM 轻量调优。
...
后者就比前者更像用户真正想要的表达。这样的样例长期积累后，可以用于改进本地润写模型。
...
因此，自进化相关数据应尽量保留在用户本机，并由用户控制是否保存、删除、导出或参与本地训练。未来如果新增云端同步、云端训练或共享样例能力，也应该明确标注数据去向，并提供显式开关。

## 自进化能力现在已经完成了吗？

当前它是重点路线和产品方向，不应理解为所有自动训练能力都已经完整完成。
