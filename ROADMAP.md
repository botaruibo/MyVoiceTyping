...
- [x] 上线 GitHub Pages landing page，承接社区和搜索流量；
- [x] 增加 30 秒试用任务，帮助新用户用虚构句快速判断是否适合；
- [x] 增加 Use cases、Alternatives、Privacy、Security、Self-evaluation、Support 等说明文档；
- [x] 增加 GitHub issue templates，覆盖 Bug、体验反馈、使用场景、AI Coding prompt 和功能建议；
- [x] 增加 Support / 反馈与支持路由页，统一分流 Issue、Discussions、Dataset 样例征集和隐私 / 安全反馈；
- [x] 补齐 GitHub repository topics，覆盖 `voice-typing`、`dictation`、`local-first`、`ai-coding`、`typeless-alternative`、`chinese-voice-input` 等发现关键词；
- [x] 增加 robots、sitemap、JSON-LD、llms.txt 和 landing 的 `llms.txt` 发现入口，方便搜索引擎和 AI crawler 理解项目定位；
- [x] 建立 Demo/GIF 追踪 issue，并准备 60 秒 Demo 脚本和传播素材；
- [x] 建立 Dataset 样例征集入口，用于收集可公开、已脱敏的 ASR 后处理失败样例。
...
- [ ] 改进粘贴到当前输入框的稳定性；
- [ ] 增加更多可复现的测试样例；
- [ ] 持续完善 FAQ、Privacy、Security、Roadmap、Support 和 Issue 模板；
- [ ] 明确 App 仓库项目级 License，并同步 README / landing / Release notes；
- [ ] 明确 self-evaluation 数据边界：本地保存、用户可控、默认不上传云端。
- [ ] 发布 15–60 秒真实 Demo / GIF，并回填 README、landing、GitHub issue 和外部社区投稿。
...
2. 用 [30 秒试用任务](docs/TRIAL_TASKS.zh-CN.md) 里的虚构句测试中文、中英混合、AI Coding prompt、隐私输入和 self-evaluation 场景；
3. 不确定该反馈到哪里时，先看 [Support / 反馈与支持](SUPPORT.md)；
4. 用 [体验反馈 / User feedback](https://github.com/botaruibo/MyVoiceTyping/issues/new/choose) 提交使用场景和输出样例；
5. 用 [Bug report / 故障报告](https://github.com/botaruibo/MyVoiceTyping/issues/new/choose) 提交可复现问题；
6. 在 [Discussions](https://github.com/botaruibo/MyVoiceTyping/discussions/2) 里反馈你希望优先改进的场景；
7. 如果你愿意帮助 self-evaluation / 自进化方向，请优先提交可公开、脱敏后的“原始转写 → 期望文本”样例；
8. 如果项目解决了你的问题，欢迎 Star 关注后续版本。
...
- Support / 反馈与支持：[SUPPORT.md](SUPPORT.md)
