...
🧾 License decision guide: https://github.com/botaruibo/MyVoiceTyping/blob/main/docs/LICENSE_DECISION.zh-CN.md  
...
- 你是否愿意给早期项目一点反馈，帮助补齐 Demo/GIF、License 和更稳定的试用体验。
...
License / 使用边界：应用代码许可证仍在确认中，见 [Issue #7](https://github.com/botaruibo/MyVoiceTyping/issues/7) 和 [License decision guide / 许可证决策指南](docs/LICENSE_DECISION.zh-CN.md)；模型页当前标注 Apache-2.0，同时请遵守上游 Qwen 模型许可；数据集由多个公开来源整理而来，使用、再分发、商业训练或公开发布模型前，请查看 Dataset 的 [DATA_LICENSE](https://github.com/botaruibo/MyVoiceTyping-Dataset/blob/main/DATA_LICENSE.md) 和 [SOURCES](https://github.com/botaruibo/MyVoiceTyping-Dataset/blob/main/docs/SOURCES.md)，并确认各原始数据源的许可证、引用要求和使用边界。
