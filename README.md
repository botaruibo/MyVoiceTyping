...
想先看输入效果，可以查看 [Demo / 使用效果示例](docs/DEMO.md) 和 [Use cases / 使用场景](docs/USE_CASES.zh-CN.md)；如果想录制、转发、投稿或在相关社区讨论里透明自荐，可以参考 [Demo assets / 演示传播素材](docs/DEMO_ASSETS.md)、[Community sharing / 社区讨论回复素材](docs/COMMUNITY_SHARING.md) 和 [Demo video script / 录屏脚本](docs/DEMO_VIDEO.md)。遇到权限、模型下载、转写质量或粘贴问题，请先查看 [FAQ / Troubleshooting](docs/FAQ.md)。
