...
想先看输入效果，可以查看 [Demo / 使用效果示例](docs/DEMO.md)；如果想录制或转发 60 秒演示，可以参考 [Demo video script / 录屏脚本](docs/DEMO_VIDEO.md)。遇到权限、模型下载、转写质量或粘贴问题，请先查看 [FAQ / Troubleshooting](docs/FAQ.md)。
...
License / 使用边界：应用代码许可证仍在确认中，见 [Issue #7](https://github.com/botaruibo/MyVoiceTyping/issues/7)；模型页当前标注 Apache-2.0，同时请遵守上游 Qwen 模型许可；数据集由多个公开来源整理而来，使用、再分发、商业训练或公开发布模型前，请确认各原始数据源的许可证、引用要求和使用边界。
