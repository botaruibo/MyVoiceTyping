...
class SenseVoiceSmall:
        ...
        **kwargs,
        ...
        if not os.path.exists(model_file):
            raise FileNotFoundError(
                f"Missing ONNX model file under {model_dir}. "
                "Desktop build only supports pre-exported ONNX models downloaded from ModelScope."
            )
    ...
    def __call__(self, wav_content: Union[str, np.ndarray, List[str]], **kwargs):
        ...
        for beg_idx in range(0, waveform_nums, self.batch_size):
            ...
            for b in range(feats.shape[0]):
                # ONNX Runtime returns numpy arrays here; decode without torch to keep packaging lean.
                x = np.asarray(ctc_logits[b, : int(encoder_out_lens[b]), :], dtype=np.float32)
                yseq = np.argmax(x, axis=-1).astype(np.int32, copy=False)
                if yseq.size:
                    keep_mask = np.empty_like(yseq, dtype=bool)
                    keep_mask[0] = True
                    keep_mask[1:] = yseq[1:] != yseq[:-1]
                    yseq = yseq[keep_mask]
    ...
    def load_data(self, wav_content: Union[str, np.ndarray, List[str]], fs: int = None) -> List:
        def load_wav(path: str) -> np.ndarray:
            ...
            if fs is not None and sample_rate != fs:
                raise ValueError(f"Only {fs} Hz wav is supported, got {sample_rate}")
            if isinstance(waveform, np.ndarray) and waveform.ndim == 2:
                if waveform.shape[1] != 1:
                    raise ValueError(
                        f"Only mono wav is supported, got {waveform.shape[1]} channels"
                    )
                waveform = waveform[:, 0]
        ...
        if isinstance(wav_content, np.ndarray):
            return [np.asarray(wav_content, dtype=np.float32)]
